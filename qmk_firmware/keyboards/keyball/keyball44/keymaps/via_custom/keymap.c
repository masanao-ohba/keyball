...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  // Right middle-right: LT(2,-)=数字レイヤー右手ホールド
  // Left top-left: LT(3,ESC)=記号レイヤー左手ホールド
  [0] = LAYOUT_universal(
    LT(3,KC_ESC), KC_Q     , KC_W     , KC_E     , KC_R     , KC_T     ,                                        KC_Y     , KC_U     , KC_I     , KC_O     , KC_P     , KC_BSPC  ,
    LCTL_T(KC_TAB), KC_A   , KC_S     , KC_D     , KC_F     , KC_G     ,                                        KC_H     , KC_J     , KC_K     , KC_L     , KC_QUOT  , LT(2,KC_MINS),
    KC_LSFT  , KC_Z     , KC_X     , KC_C     , KC_V     , KC_B     ,                                        KC_N     , KC_M     , MT(MOD_LCTL|MOD_RCTL,KC_COMM), KC_DOT, KC_SLSH, MT(MOD_LSFT|MOD_RSFT,KC_LBRC),
              TG(2)    , MO(2)    , MT(MOD_LGUI,KC_SPC), MT(MOD_LALT,KC_ENT), KC_CAPS,                  MT(MOD_LALT|MOD_RALT,KC_ENT), MT(MOD_LGUI|MOD_RGUI,KC_SPC), MT(MOD_LCTL|MOD_RCTL,KC_LNG2), KC_RALT, KC_INT1
  ),
  ...
  [3] = LAYOUT_universal(
    ...
    KC_LSFT  , KC_NO    , KC_NO    , KC_NO    , KC_NO    , KC_NO    ,                                        KC_NO    , KC_NO    , KC_RBRC  , KC_NUHS  , KC_SLSH  , S(KC_INT3),
              _______  , _______  , MT(MOD_LGUI,KC_SPC), KC_LSFT  , _______  ,                   KC_RSFT  , MT(MOD_LGUI|MOD_RGUI,KC_SPC), _______       , S(KC_INT1), _______
  ),
};
// clang-format on
